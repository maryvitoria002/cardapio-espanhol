...
try {
    ...
    if (empty($_POST['primeiro_nome']) || empty($_POST['segundo_nome']) || 
        empty($_POST['email']) || empty($_POST['telefone']) || 
        empty($_POST['acesso']) || empty($_POST['senha'])) {
        throw new Exception('Todos os campos obrigatórios devem ser preenchidos');
    }
    
    // Verificar se o email já existe
    if ($crudFuncionario->emailExists($_POST['email'])) {
    ...
    // Criar funcionário
    if ($crudFuncionario->createAdmin()) {
