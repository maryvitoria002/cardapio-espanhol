...
class Crud_usuario extends Usuario{
    ...
    */
    ...
    public function emailExists($email, $id_usuario = null) {
        try {
            if ($id_usuario) {
                ...
                // Para criação - verificar se email existe
                $sql = "SELECT COUNT(*) as total FROM `{$this->tabela}` 
                        WHERE email = :email";
                $database = new Database();
                $stmt = $database->prepare($sql);
    ...
    // Métodos setter para compatibilidade com formulários admin
    ...
    public function setSegundoNome($segundo_nome) {
        $this->setSegundo_nome($segundo_nome);
    }
    
    public function setEndereco($endereco) {
        // Método para compatibilidade - campo endereco não existe mais
        return true;
    }
    
    public function setImagemUsuario($imagem) {
        $this->setImagem_perfil($imagem);
    }
    ...
    public function uploadImagem($file) {
        ...
        $uploadDir = __DIR__ . '/../../images/usuarios/';
